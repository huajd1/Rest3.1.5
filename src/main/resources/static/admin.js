...
function getUserInformationPage() {
    fetch(url)
        ...
        .then(data => {
            ...
            document.getElementById('adminTableBody').innerHTML = output;
...
function getAdminPage() {
    fetch(url)
        ...
        .then(users => {
            if (users.length > 0) {
                const user = users[0]; // Выбираем первого пользователя из массива
...
document.getElementById('newUserForm').addEventListener('submit', (e) => {
    ...
    for (let i = 0; i < role.options.length; i++) {
        if (role.options[i].selected) {
            rolesAddUser.push({id: role.options[i].value, firstname: 'ROLE_' + role.options[i].innerHTML})
